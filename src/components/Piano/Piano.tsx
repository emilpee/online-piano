...
const Piano: FunctionComponent = () => {
    ...
    return (
        <div className="piano">
            <PianoKeysContainer>
                <PianoVolume id="inc" handlePianoVolume={handlePianoVolume}>+</PianoVolume>
                <PianoVolume id="dec" handlePianoVolume={handlePianoVolume}>-</PianoVolume>
                {pianoData.map((pianoKey) => <PianoKey onKeyClick={playNote} key={pianoKey.key} id={pianoKey.key} className={`piano-key ${pianoKey.type}`} />)}
            </PianoKeysContainer>
        </div>
    )
}
